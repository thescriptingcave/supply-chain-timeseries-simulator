...
def test_reefer_temperature_is_written_to_telemetry_not_vehicle_state() -> None:
    ...
    assert not hasattr(v, "cargo_temp_c")
    assert any(
        sample.row.cargo_temp_c == 12.0
        for sample in result.telemetry_samples
    )
    assert any(
        sample.row.cargo_temp_c == 5.0
        for sample in result.telemetry_samples
    )
    assert all(
        sample.row.cargo_humidity_pct == 60.0
        for sample in result.telemetry_samples
    )
