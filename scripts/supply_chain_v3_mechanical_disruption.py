"""Persisted deterministic mechanical-breakdown integration gate for Supply Chain v3.

Run from repository root:

    uv run python -m scripts.supply_chain_v3_mechanical_disruption

This scenario persists one shipment with a vehicle-scoped mechanical breakdown,
including zero-speed telemetry, causal ETA updates, disruption boundary events,
    ...
    try:
        ...
        run_id = writer.create_simulation_run(
            SimulationRunRecord(
                ...
                status="STARTED",
                metadata_json={
                    "purpose": "mechanical-breakdown-integration-gate",
                    "breakdown_start_offset_minutes": 30,
        ...
        mechanical_events = [
            ...
            if event.cause_code == "MECHANICAL_BREAKDOWN"
        ]
        ...
        print(
            "[Supply Chain v3] Mechanical disruption integration passed: "
            f"run_id={run_id}, "
            f"shipment_id={shipment_id}, "
            f"fleet_telemetry_rows={len(result.telemetry_samples)}, "
            f"events={len(result.events)}, "
            f"mechanical_causal_events={len(mechanical_events)}, "
            f"zero_speed_samples={stopped_samples}, "
