"""Persist a 2-day Supply Chain v3 validation dataset.

This runner is intentionally a calibration workload, not the final long-run
production generator. It produces a dense but deterministic mix of v3 behaviors
so the resulting TimescaleDB dataset can be inspected with SQL and Grafana.

Run from repository root:

    uv run python -m scripts.generate_supply_chain_v3_validation --days 2 --seed 42
"""
...
from generators.supply_chain.run_config import validation_profile
...
def build_validation_categories(
    *,
    shipment_count: int,
    vehicle_count: int,
    seed: int,
    ...
    config = validation_profile(days=days, seed=seed)
    ...
    # Three shipments are reserved for the shared warehouse contention gate.
    categories.extend(["contention"] * min(3, shipment_count))

    remaining = shipment_count - len(categories)

    # Use the planner's seeded totals, but make each validation shipment own one
    # primary behavior so the resulting data is easy to reason about.
    for name in ("traffic", "weather", "mechanical", "reefer"):
        take = min(totals[name], remaining)
        categories.extend([name] * take)
        remaining -= take

    fuel_count = min(max(1, round(shipment_count * 0.08)), remaining)
    ...
    if len(categories) != shipment_count:
        raise RuntimeError("validation workload planning produced wrong count")
    ...
    try:
        ...
        run_id = writer.create_simulation_run(
            SimulationRunRecord(
                ...
                simulation_end=end,
                configuration_version="v3-validation",
                status="STARTED",
                metadata_json={
                    "purpose": "v3-2-day-validation-dataset",
        ...
        # First three shipments: shared warehouse contention.
        if categories[:3] == ["contention", "contention", "contention"]:
            route = routes[0]
        ...
        print("[Supply Chain v3] 2-day validation dataset passed")
