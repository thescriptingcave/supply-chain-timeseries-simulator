"""Persisted deterministic traffic-disruption integration gate for Supply Chain v3.

Run from repository root:

    uv run python -m scripts.supply_chain_v3_traffic_disruption

The scenario persists one shipment with a route-scoped traffic disruption and
verifies that the event/ETA/telemetry chain is represented in TimescaleDB.
"""
    ...
    try:
        ...
        run_id = writer.create_simulation_run(
            SimulationRunRecord(
                ...
                status="STARTED",
                metadata_json={
                    "purpose": "traffic-disruption-integration-gate",
                    "traffic_start_offset_minutes": 30,
                    "traffic_duration_minutes": 60,
                    "traffic_speed_factor": 0.50,
                },
            )
        )
        ...
        with conn.cursor() as cur:
            ...
            vehicle = load_vehicle(cur, origin=origin)
        ...
        disruption = traffic_disruption(
            disruption_id=f"TRAFFIC-{run_id}-1",
        ...
        writer.insert_shipments([item.shipment])
        ...
        writer.insert_events(item.events)
        ...
        traffic_events = [
            ...
            if event.cause_code == "TRAFFIC_CONGESTION"
        ]

        print(
            "[Supply Chain v3] Traffic disruption integration passed: "
            f"run_id={run_id}, "
            f"shipment_id={shipment_id}, "
            f"fleet_telemetry_rows={len(item.telemetry_samples)}, "
            f"events={len(item.events)}, "
            f"traffic_causal_events={len(traffic_events)}, "
            f"actual_arrival={item.shipment.actual_arrival.isoformat()}"
        )
